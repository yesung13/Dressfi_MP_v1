		...
		$.ajax({
					...
					success : function(resultList) {
						...
						for (var i = 0; i < resultList.length; i++) {
							...
							html += '<img src="${pageContext.request.contextPath}/resources/img/'+result.MF_PROIMG+'" alt="사진" class="profile mr-3 mt-3 rounded-circle border img-fluid" style="width:50px; height: 50px;" />';
							html += '</a>';
							html += '</div>';//카드헤더부분
							...
							if (result.MF_DEPT == '1') {
								html += '도로시공장';
							} else if (result.MF_DEPT == '2') {
								html += '다이마루공장';
							} else if (result.MF_DEPT == '3') {
								html += '직기공장';
							} else if (result.MF_DEPT == '4') {
								html += '특수공장';
							}
							html += '</small>';
							html += '</div>';
			...
			<div class="d-flex">
				...
				<div class=" mr-1">
					<select class="custom-select custom-select-sm border border-secondary mb-3" id="select2">
						<option value="0" selected>공장분야</option>
