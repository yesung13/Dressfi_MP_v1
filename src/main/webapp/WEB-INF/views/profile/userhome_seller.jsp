	...
	function fAction(where) {
		...
		case 1:
			if (confirm("팔로우를 취소하시겠습니까?")) {
				location.href = "delfl.do?name=${profile.mc_name}&&i_mdiv=${i_mdiv}&i_mnum=${i_mnum}";
			}
			break;

		case 2:
			if (confirm("이 사용자를 팔로우 하시겠습니까?")) {
				location.href = "incfl.do?name=${profile.mc_name}&&i_mdiv=${i_mdiv}&i_mnum=${i_mnum}";
			}
			break;
		}
	}
			...
			<div class="row mt-4">
				<div class="col-sm-12 col-md-12 col-xl-4 mb-4 mypage-fill pofill-bor">
					...
					<div class="d-flex profileimg mb-2">
						<img src="<c:url value="/resources/img/${profile.mc_proimg}/"/>" alt="프로필사진" width="150" class="rounded-circle" />
					</div>

					<!-- 닉네임  -->
					<div class="d-flex justify-content-start mb-1">
						<h3>
							<span class="font-weight-bold">${profile.mc_name}</span>
						</h3>
						<%-- &#149; ${profile.mc_dept } --%>
					</div>
					...
					<div class="d-flex mb-1">
						<a class="text-decoration-none" href="${pageContext.request.contextPath}/profile/followerView.do?userName=${profile.mc_name}">
							<span class="text-black-50 font-weight-bold">팔로워</span>
							<span class="sr-only">팔로워 수 코딩자리</span>

						</a>
						<a class="text-decoration-none" href="${pageContext.request.contextPath}/profile/followingView.do?userName=${profile.mc_name}">
					...
					<div class="d-flex justify-content-center mb-3">
						...
						<a class="snschat text-decoration-none text-muted" href="${profile.mc_kakaourl}" data-toggle="tooltip" data-placement="bottom" title="카카오톡 오픈채팅">
							<img src="${pageContext.request.contextPath}/resources/images/svg/open-kakao.svg">
						</a>
					</div>
					<div class="mb-4">
						<strong class="text-center text-muted font-weight-bold">${profile.mc_introduce }</strong>
					</div>
					...
					<table class="table table-borderless table-sm">
						<tr>
							...
							<td colspan="2">
								<span class="">${profile.mc_phone}</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">
									<a href="${profile.mc_homepage }">${profile.mc_homepage }</a>
								</span>
							</td>
						</tr>
						<tr>
							...
							<td colspan="2">
								<span class="">${profile.mc_div}</span>
							</td>
						</tr>
						<tr>
							<th>
								<span class="">의류분야</span>
							</th>
							<td colspan="2">
								<span class="">${profile.mc_dept }</span>
							</td>
						</tr>
						<tr>
							<th>
								<span class="">세부분야</span>
							</th>
							<td colspan="2">
								<span class="">${profile.mc_subdept}</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.mc_business }</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.mc_bnum}</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.mc_bname }</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.mc_badd1 }${profile.mc_badd2 }</span>
							</td>
						</tr>
						<tr>
							<th>
				...
				<div class="col-sm-12 col-md-12 col-xl-8  mypage-fill">
					...
					<ul class="nav nav-pills pople">
						...
						<li class="nav-item">
							<a class="nav-link" href="${pageContext.request.contextPath}/profile/portfoliolist_design.do?userId=${profile.mc_userId}">포트폴리오 </a>
						</li>
						<li class="nav-item">
							<a class="nav-link" href="${pageContext.request.contextPath}/profile/knowhow_design.do?userId=${profile.mc_userId}">노하우 </a>
						</li>
					</ul>
					<!--포트폴리오 게시물 -->
