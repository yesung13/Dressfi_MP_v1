...
<script type="text/javascript">
	...
	function deleteReply(rhno, hno){
		if(confirm("선택하신 댓글을 삭제하시겠습니까?")){
			location.href = "deletereply.do?rhno=" + rhno + "&hno=" + hno;
		}
	}
	
	function recommendReply(rhno, hno){
		if(confirm("선택하신 댓글을 추천하시겠습니까?")){
			location.href = "likeknowreply.do?rhno=" + rhno + "&hno=" + hno;	
		}
	}
	...
	function moveAction(where){
		...
		case 1:
			if(confirm("글을 삭제하시겠습니까?")){
				location.href ="delknow.do?hno=${knowhow.hno}";
			}
			break;
		...
		case 3:
			location.href = "/dressfi/board/knowhow.do";			
			break;
			
		case 4:
			if(confirm("이 글이 좋아요?")){
				location.href = "likeknow.do?hno=${knowhow.hno}";	
			}	
			break;
		}
	}
...
<body>
	...
	<section>
		<div class="container margin-30 mb-4">
			<div class="row mt-3">
					...
					<div class="mt-1  photo-view-fill mb-5">
						<div class="col-xl-12">
							<div class="card profile-card-2">
								<p class="font-weight-bold" style="font-size: 1.2em">${knowhow.h_title}</p>
								<div class="">
									<a href="#">
										<img class="img-fluid rounded border" src="${knowhow.h_fileloc }" alt="사진" style="width: 1000px; height: 700px" />
									</a>
								</div>
								<p class="text-justify text-break mt-2">${knowhow.h_text}</p>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="col-sm-12 col-md-12 col-xl-4 photo-fill-2 mt-1">
					...
					<div class="d-flex margin-30 ">
						<button type="button" class="btn btn-light btn-small border w-100 mr-1" onclick="moveAction(4)">
							<i class="far fa-heart"></i>
							<span class="text-info">${knowhow.h_likecnt }</span>
						</button>

						<!-- 조회수 부분이라 버튼 동작 안함 -->
						<button type="button" class="btn btn-light btn-small border w-100" disabled="disabled" onclick="">
							<i class="fas fa-eye"></i>
							<span class="text-info">${knowhow.h_viewcnt }</span>
						</button>
						<!--//조회수  -->

						<button class="btn btn-sm dropdown show dropdown-toggle" type="button" id="textmdBtn" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
							<i class="fas fa-ellipsis-v"></i>
						</button>
						<div class="dropdown-menu dropdown-menu-right" aria-labelledby="textmdBtn">
							<a class="dropdown-item" onclick="moveAction(2)">수정</a>
							<a class="dropdown-item" onclick="moveAction(1)">삭제</a>
						</div>
					</div>
					...
					<div class="d-flex flex-row justify-content-between margin-30">
						<div class="">
							<a href="${pageContext.request.contextPath}/profile/userhome.do?i_mnum=${knowhow.h_mnum}&i_mdiv=${knowhow.h_mdiv}">
								<img src="<c:url value="/resources/img/${user.proimg}/"/>" alt="프로필사진" style="width:50px; height:50px;" class="img-fluid boarder rounded-circle ml-1">
								<span class="ml-1 text-dark">
									<strong>${knowhow.h_name}</strong>
								</span>
							</a>
						</div>
						<div class="invisible">
					...
					<c:forEach items="${knowhowList}" var="item">
						<c:if test="${knowhow.hno != item.hno }">
							<div class="col-sm-6 col-md-6 col-lg-6 ">
								<div class="card profile-card-2">
									<div class="">
										<a href="${pageContext.request.contextPath}/profile/knowhowview.do?hno=${item.hno}">
											<img class="img-fluid rounded border" src="${item.h_fileloc }" alt="${h_fileloc }" style="width: 350px; height: 150px" />
										</a>
									</div>
								</div>
							</div>
		...
		<div class="container photo-margin border-top">
			<div class="row ml-3 mt-4">
				<div class="col-12 mb-1">
					<h5>
						<span class="font-weight-bold mr-1">댓글</span>
						<span class="text-info text-bold">${knowhow.h_replycnt}</span>
					</h5>
				</div>
				<!-- 내 댓글 -->
				<form class="col-12 mb-4" action="kwritereply.do" method="post">
					<div class="orm-group ml-auto d-flex justify-content-start">
						<div class="mr-2">
							<img src="<c:url value="/resources/img/${p_img}/"/>" alt="프로필사진" width="30" height="30" class="rounded-circle">
						</div>
						<div class="d-flex justify-content-start col-sm-12 col-xl-9">
							<input type="hidden" id="rh_name" name="rh_name" value="${userName}" />
							<input type="hidden" id="hno" name="hno" value="${knowhow.hno}" />
							<input type="text" name="rh_text" id="rh_text" class="form-control" id="exampleInputEmail1" placeholder="칭찬과 격려의 글은 작성자에게 큰힘이 됩니다.">
							<input type="submit" value="확인" class="btn btn-light ml-2 mr-md-4 mr-sm-4" />
						</div>
					</div>
				</form>
				<!--// 내 댓글  -->
				...
				<c:forEach var="comment" items="${replyList}">
					<div class="col-12 ">
						<div class=" py-2">
							<%-- <img src="<c:url value="/resources/img/${p_img}/"/>" alt="프로필사진" width="30" height="30" class="rounded-circle"> --%>
							<span class="font-weight-bold mr-2">${comment.rh_name }</span>
							<span class=""> ${comment.rh_text}</span>
							<div class="small text-888 ml-5 pl-4">
								<span>${comment.rh_regdate }</span>
								<a href="#" onclick="recommendReply(${comment.rhno}, ${comment.hno})">
									<span class="ml-2">
										<i class="far fa-heart"></i>&nbsp;좋아요&nbsp;${comment.rh_likecnt }
									</span>
								</a>

								<c:if test="${comment.rh_name == userName}">
									<a href="#" onclick="deleteReply(${comment.rhno}, ${comment.hno})">
