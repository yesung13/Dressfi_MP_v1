	...
	function fAction(where) {
		...
		case 1:
			if (confirm("팔로우를 취소하시겠습니까?")) {
				location.href = "delfl.do?name=${profile.md_name}&&i_mdiv=${i_mdiv}&i_mnum=${i_mnum}";
			}
			break;

		case 2:
			if (confirm("이 사용자를 팔로우 하시겠습니까?")) {
				location.href = "incfl.do?name=${profile.md_name}&&i_mdiv=${i_mdiv}&i_mnum=${i_mnum}";
			}
			break;
		}
	}
			...
			<div class="row mt-4">
				<div class="col-sm-12 col-md-12 col-xl-4 mb-4 mypage-fill pofill-bor">
					...
					<div class="d-flex justify-content-between mb-4 invisible">
						<a class="text-secondary text-decoration-none " href="${pageContext.request.contextPath}/profile/pmodify.do?userId=${userId}">
							<i class="fas fa-cog"></i>
							<span> 프로필 수정</span>
						</a>
						<!-- 글쓰기  -->
						<a class="text-secondary text-decoration-none  " href="${pageContext.request.contextPath}/profile/portfoliowrite.do?userId=${userId}">
					...
					<div class="d-flex profileimg mb-2">
						<img src="<c:url value="/resources/img/${profile.md_proimg}/"/>" alt="프로필사진" width="150" class="rounded-circle" />
					</div>

					<!-- 닉네임  -->
					<div class="d-flex justify-content-start mb-1">
						<h3>
							<span class="font-weight-bold">${profile.md_name}</span>
						</h3>
						<%-- &#149; ${profile.md_dept } --%>
					</div>
					...
					<div class="d-flex justify-content-center mb-3">
						<c:if test="${folyn eq 'n'}">
							<button class="btn btn-secondary bg-ddd text-center text-black font-weight-bold w-100 mr-1" onclick="fAction(2)">팔로우</button>

						</c:if>
						<c:if test="${folyn eq 'y'}">
							<button class="btn btn-secondary active bg-ddd text-center text-black font-weight-bold w-100 mr-1" onclick="fAction(1)">팔로우 취소</button>

						</c:if>
						<a class="snschat text-decoration-none text-muted" href="${profile.md_kakaourl}" data-toggle="tooltip" data-placement="bottom" title="카카오톡 오픈채팅">
							<img src="${pageContext.request.contextPath}/resources/images/svg/open-kakao.svg">
						</a>
					</div>
					<div class="mb-4">
						<strong class="text-center text-muted font-weight-bold">${profile.md_introduce }</strong>
					</div>
					...
					<div class="card-text">
						...
						<strong class="pl-1"><b>${profile.md_phone}</b></strong>
						<!-- ${loginUser.phone}  -->
					</div>
					
					<div class="card-text mt-2">
						<h6>
							<strong class="">홈페이지</strong>
						</h6>
						<strong class="pl-1"><b>${profile.md_homepage }</b></strong>
					</div>
					
					<div class="card-text mt-2">
						<h6>
							<strong class="">경력</strong>
						</h6>
						<strong class="pl-1"><b>${profile.md_career }</b></strong>
					</div>
					
					<div class="card-text mt-2">
						<h6>
							<strong class="">사업자등록</strong>
						</h6>
						<strong class="pl-1"><b>${profile.md_business }</b></strong>
					</div>
					
					<div class="card-text mt-2">
						<h6>
							<strong class="">사업자번호</strong>
						</h6>
						<strong class="pl-1"><b>${profile.md_bname }</b></strong>
					</div>
					
					<div class="card-text mt-2">
						<h6>
							<strong class="">상호명</strong>
						</h6>
						<strong class="pl-1"><b>${profile.md_bname }</b></strong>
					</div>
					...
					<div class="card-text mt-2">
						<h6>
							<strong class="">사업장주소</strong>
						</h6>
						<strong class="pl-1"><b>${profile.md_badd1 }${profile.md_badd2 }</b></strong>
					</div>
					
					<div class="card-text mt-2">
						<h6>
							<strong class="">활동지역</strong>
						</h6>
						<strong class="pl-1"><b>${ano1},${ano2},${ano3}</b></strong>
					</div>
					<!-- 
					<div class="d-inline-block mb-2">
						<p>
							<b>위치</b> <span>서울특별시 강서구 영등포구 000000</span> <br> <b>경력</b> <span>10~15년</span> <br> <b>AS</b> <span>12개월 </span>
						</p>
						<p class="label-1">서울</p>
						<p class="label-1">대전</p>
						<p class="label-1">경기</p>
						<p class="label-1">표준</p>
						<p class="label-1">표준계약서</p>
						<p class="label-1">표준계약서</p>
 ...
 --%>
					<table class="table table-borderless table-sm">
						<tr>
							...
							<td colspan="2">
								<span class="">${profile.md_phone}</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">
									<a href="${profile.md_homepage }">${profile.md_homepage }</a>
								</span>
							</td>
						</tr>
						<tr>
							<th>
								<span class="">디자인분야</span>
							</th>
							<td colspan="2">
								<span class="">${profile.md_dept }</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.md_career }</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.md_business }</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.md_bnum}</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.md_bname }</span>
							</td>
						</tr>
						<tr>
							<th>
							...
							<td colspan="2">
								<span class="">${profile.md_badd1 }${profile.md_badd2 }</span>
							</td>
						</tr>
						<tr>
							<th>
				...
				<div class="col-sm-12 col-md-12 col-xl-8  mypage-fill">
					...
					<ul class="nav nav-pills pople">
						<li class="nav-item">
							<a class="nav-link" href="${pageContext.request.contextPath}/profile/userhome.do?i_mdiv=${i_mdiv}&i_mnum=${i_mnum}">모두보기</a>
						</li>
						<li class="nav-item">
							<a class="nav-link active" href="${pageContext.request.contextPath}/profile/portfoliolist_design.do?userId=${profile.md_userId}">포트폴리오 </a>
						</li>
						<li class="nav-item">
							<a class="nav-link" href="${pageContext.request.contextPath}/profile/knowhow_design.do?userId=${profile.md_userId}">노하우 </a>
						</li>
					</ul>
					<!-- 이미지게시물  -->
					<div class="container">
						<div class="row">
							<c:forEach items="${portfolioList}" var="item">
								<div class="col-sm-12 col-md-12 col-xl-4 px-2">
									<a href="${pageContext.request.contextPath}/profile/userpphotoview.do?ino=${item.ino}&i_mnum=${item.i_mnum}&i_mdiv=${item.i_mdiv}">
										<img class="img-fluid rounded border" src="${item.i_fileloc }" alt="${item.i_fileloc }" style="width: 400px; height: 180px;" />
									</a>
									<p class="sl-title d-flex justify-content-around">
										<a href="#" class="card-link">
											<i class="far fa-heart"></i> ${item.i_likecnt }
										</a>
										<a href="#" class="card-link">
											<i class="fas fa-eye"></i> ${item.i_viewcnt }
										</a>
										<a href="#" class="card-link">
											<i class="far fa-comment-alt"></i> ${item.i_replycnt }
										</a>
									</p>
									<div class="text-break pb-5">
										<span>${item.i_title }</span>
									</div>
								</div>
							</c:forEach>
						</div>
