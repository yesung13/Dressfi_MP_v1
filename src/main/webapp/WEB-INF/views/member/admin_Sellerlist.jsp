...
<script type="text/javascript">
	function fnCmdDelete(sampleNo) {
		document.sellerForm.mcno.value = sampleNo;
		document.sellerForm.action = "MSdeletec.do";
	...
	<section>
		<div class="container mt-1">
			...
			<ul class="nav nav-tabs flex-column flex-sm-row">
				...
				<li class="nav-item">
					<a class="flex-sm-fill text-sm-center nav-link active" href="/dressfi/member/adminMClist.do">도소매회원</a>
				</li>
			</ul>
			<!-- Tab panes -->
			<div class="tab-content">
				<!-- 도소매회원 리스트-->
				<div id="pro3" class="container">
					<h4 class="font-weight-bolder mt-4 mb-4">도소매회원 리스트</h4>
					<form:form id="sellerForm" name="sellerForm" method="post" class="all_layout">
						<!--POST방식  -->
						...
						<table class="table table-bordered mt-3">
							...
							<tbody>
								<c:forEach var="result" items="${slist}" varStatus="status">
									<tr align="center">
										<td>${result.mcno }</td>
										<td>${result.mc_userId }</td>
										<td>${result.mc_name }</td>
										<td>${result.mc_phone }</td>
										<td>${result.mc_homepage}</td>
										<td>${result.mc_kakaourl}</td>
										<td>${result.mc_joindate}</td>
										<td>
											<a href="#" onclick="fnCmdDelete('${result.mcno}')" class="btn btn-info">삭제</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
