...
<script type="text/javascript">
	function fnCmdDelete(sampleNo) {
		document.factoryForm.mfno.value = sampleNo;
		document.factoryForm.action = "MFdeletec.do";
		...
		<nav class="navbar sticky-top navbar-expand-lg navbar-light">
			<div class="container">
				<a class="navbar-brand mb-0 h1" href="<%=request.getContextPath()%>/member/adminlist.do">
					<img src="${pageContext.request.contextPath}/resources/images/svg/clothing-hanger.svg" class="d-inline-block align-top" alt="logo">
	...
	<section>
		<div class="container mt-1">
			...
			<ul class="nav nav-tabs flex-column flex-sm-row">
				...
				<li class="nav-item">
					<a class="flex-sm-fill text-sm-center nav-link" href="/dressfi/member/adminMDlist.do">디자이너회원</a>
				</li>
				<li class="nav-item">
					<a class="flex-sm-fill text-sm-center nav-link active" href="/dressfi/member/adminMFlist.do">공장회원</a>
				</li>
				<li class="nav-item">
			...
			<div class="tab-content">
				<!-- 공장회원 리스트-->
				<div id="pro2" class="container">
					<h4 class="font-weight-bolder mt-4 mb-4">공장회원 리스트</h4>
					<form:form id="factoryForm" name="factoryForm" method="post" class="all_layout">
						<!--POST방식  -->
						<input type="hidden" name="mfno" />
						...
						<table class="table table-bordered mt-3">
							...
							<tbody>
								<c:forEach var="result" items="${flist}" varStatus="status">
									<tr align="center">
										<td>${result.mfno }</td>
										<td>${result.mf_userId }</td>
										<td>${result.mf_name }</td>
										<td>${result.mf_phone }</td>
										<td>${result.mf_homepage}</td>
										<td>${result.mf_kakaourl}</td>
										<td>${result.mf_joindate}</td>
										<td>
											<a href="#" onclick="fnCmdDelete('${result.mfno}')" class="btn btn-info">삭제</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
